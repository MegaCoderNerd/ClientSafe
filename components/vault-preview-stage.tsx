...
export function VaultPreviewStage({
  ...
  useEffect(() => {
    ...
    void node
      ...
      .catch(() => {
        /* Overlay is portaled to document.body, so it still covers the real viewport. */
      });
    ...
    function onFullscreenChange() {
      if (enteredFullscreen && !document.fullscreenElement) setExpanded(false);
    }
  ...
  const demoFrame = (fullscreen: boolean) =>
    demoSrc ? (
      <iframe
        ...
        className={
          fullscreen
            ...
            : "h-[min(70vh,36rem)] min-h-[16rem] w-full bg-white sm:h-auto sm:min-h-0 sm:aspect-video"
        }
      />
    ) : null;
    ...
    <section className="glass-card p-4 sm:p-6">
      ...
      <div className="mt-4">
        {tab === "demo" && demoSrc ? (
          ...
          <div className="relative aspect-video min-h-[12rem] w-full overflow-hidden rounded-lg border bg-slate-950">
            {preview}
          </div>
        )}
      </div>
        ...
        ? createPortal(
            ...
            >
              <div className="absolute inset-0">
                {tab === "demo" ? demoFrame(true) : preview}
              </div>
              <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-start justify-between gap-3 bg-gradient-to-b from-black/70 to-transparent p-4 pt-[max(1rem,env(safe-area-inset-top))]">
                ...
                <Button
                  ...
                  className="pointer-events-auto border-white/20 bg-black/50 text-white hover:bg-white/15 hover:text-white"
                >
                  Close
                </Button>
              </div>
